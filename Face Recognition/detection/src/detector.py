    ...
    def detect_objects(self, frame):
        ...
        for i, (bbox, point, score) in enumerate(zip(bboxes, points, scores)):
            ...
            if f_score != 0:
                ...
                cv2.rectangle(org_img, (int(px1), int(py1)), (int(px2), int(py2)), (0, 255, 0), line_thick)                
                cv2.imwrite('./output/output_image.jpg', org_img)

                # detect được cả mặt thì mới lưu object -> nếu không sẽ ignoreignore
                object = {
                    'person_id': i,
